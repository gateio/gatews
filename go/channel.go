...
func (ws *WsService) APIRequest(channel string, payload any) error {
	var err error
	...
	return ws.apiRequest(channel, payload)
}
...
func (ws *WsService) apiRequest(channel string, payload any) error {
	req := Request{
		...
		Channel: channel,
		Event:   API,
		Payload: ws.generateAPIRequest(channel, payload),
	}
...
func (ws *WsService) generateAPIRequest(channel string, placeParam any) any {
	// paramJson, _ := json.Marshal(placeParam)
	dataTime := time.Now().Unix()
	// message := fmt.Sprintf("api\n%s\n%s\n%d", channel, paramJson, dataTime)

	// Construct the request data
	reqHeader := json.RawMessage(`{"X-Gate-Channel-Id":"T-zyy131209"}`)
	reqParam, _ := json.Marshal(placeParam)

	message := fmt.Sprintf("api\n%s\n%s\n%d", channel, reqParam, dataTime)
	// fmt.Println("message --->", message)
	// Create HMAC with SHA512
	h := hmac.New(sha512.New, []byte(ws.conf.Secret))
	...
	apiReq := APIReq{
		ApiKey:    ws.conf.Key,
		Signature: signature,
		Timestamp: strconv.Itoa(int(dataTime)),
		ReqId:     "test_1",
		ReqHeader: reqHeader,
		ReqParam:  reqParam,
	}

	// Marshal the entire request structure to JSON
	// dataParam, _ := json.Marshal(apiReq)
	// fmt.Printf("channel: %s, sub_str: %s\n", channel, dataParam)
	return apiReq
